...
// Return: true on error.
bool pack_pixels(int32_t width, int32_t height, const pixel_t *data, uint8_t **ppDataOut, uint32_t *len)
{
    uint32_t buf_size = sizeof(width) + sizeof(height) + width * height * sizeof(pixel_t);
    ...
    ptr += sizeof(height);
    memcpy(ptr, data, width * height * sizeof(pixel_t));
                     ...
                     const int nx, const int ny, const float sigma)
{
    ...
    double *kernel = malloc(n * n * sizeof(double)); // variable length array
